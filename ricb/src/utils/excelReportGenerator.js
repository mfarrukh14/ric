...
const generateDemandReport = (reportData) => {
  ...
  const categoryTypes = items.map(item => item.category?.toLowerCase() || '');
...
const generateTenderReport = (tenderData) => {
  ...
  tenderData.items?.forEach((item, index) => {
    ...
    if (isPrimarilyPharmaceutical) {
      const genericName = item.name || item.description || 'N/A';
      const strength = item.specifications || '';
      const dosageForm = item.unit || 'Unit';
      ...
      const equipmentName = item.name || item.description || 'N/A';
      const specification = item.specifications || '';
      const unit = item.unit || 'Unit';
